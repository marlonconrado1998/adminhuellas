...
angular.module("ngWizardTemplate.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("ngWizardTemplate.html", '<form name="Form" id="form" novalidate>'+
    '<div id="wizard" class="swMain">' +
        '<ul>' +
            '<li ng-repeat="step in steps">' +
            '<a href  ng-class="{\'selected\' : getCurrentStep() == step, \'done\' : getStepState(step) == stepStatesEnum.complete}" ng-click="goToStepByReference(step)">'+
            '<div class="stepNumber">{{$index+1}}</div>'+
            '<span class="stepDesc text-small">{{step.title}}</span>'+
            '</a>'+
            "</li>" +
        "</ul>" +
        ...
        '<div class="container">' +
            '<div class="row">' +
                ...
                '<div class="row" ng-show="!isSubmittable()">' +
                    ...
                    "</div>" +
                    '<div class="col-xs-6">'+
                       '<button class="next btn btn-primary btn-block submit animate fade-in-out" ng-class="{disabled: !hasNext()}" ng-click="goToNext()"> {{nextString}} <i class="fa fa-arrow-circle-right"></i></button>'+            
                    "</div>" +
                "</div>" +
            "</div>" +
        "</div>" +
